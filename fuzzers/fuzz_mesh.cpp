...
extern "C" auto LLVMFuzzerInitialize(int* argc, char*** argv) -> int // NOLINT
{
    ...
    if (g_ext.empty()) {
        std::println(stderr, "Usage: fuzz_mesh --ext=<[FORMAT]>");
        pl = nullptr;
        std::exit(1);
    }
    if (locate_service<gfx::mesh_loader::factory>().create(g_ext) == nullptr) {
        std::println(stderr, "Error: no loader registered for '{}'", g_ext);
        pl = nullptr;
        std::exit(1);
    }
    return 0;
